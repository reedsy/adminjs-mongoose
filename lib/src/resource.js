...
class Resource extends adminjs_1.BaseResource {
    ...
    static isAdapterFor(MoongooseModel) {
        return (0, lodash_1.get)(MoongooseModel, 'base.constructor.name') === 'Mongoose';
    }
    databaseName() {
    ...
    async count(filters = null) {
        if (Object.keys((0, convert_filter_1.convertFilter)(filters)).length > 0) {
            return this.MongooseModel.count((0, convert_filter_1.convertFilter)(filters));
        }
    ...
    async find(filters = {}, { limit = 20, offset = 0, sort = {} }) {
        ...
        const mongooseObjects = await this.MongooseModel
            .find((0, convert_filter_1.convertFilter)(filters), {}, {
    ...
    async create(params) {
        ...
        try {
            ...
            if (error.name === MONGOOSE_VALIDATION_ERROR) {
                throw (0, create_validation_error_1.createValidationError)(error);
            }
            if (error.code === MONGOOSE_DUPLICATE_ERROR_CODE) {
                throw (0, create_duplicate_error_1.createDuplicateError)(error, mongooseDocument.toJSON());
            }
            throw error;
        }
    ...
    async update(id, params) {
        ...
        try {
            ...
            if (error.code === MONGOOSE_DUPLICATE_ERROR_CODE) {
                throw (0, create_duplicate_error_1.createDuplicateError)(error, unflattedParams);
            }
            ...
            if (error.name === MONGOOSE_CAST_ERROR) {
                throw (0, create_cast_error_1.createCastError)(error);
            }
            throw error;
        }
    }
